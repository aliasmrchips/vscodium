...
export APP_NAME="Dekstra"
export ASSETS_REPOSITORY="Dekstra/dekstra"
export BINARY_NAME="codium"
export CI_BUILD="no"
export GH_REPO_PATH="Dekstra/dekstra"
export ORG_NAME="Dekstra"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="Dekstra/dekstra-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-dekstra
    else
      echo "{}" > ~/.gyp/include.gypi.pre-dekstra
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-dekstra ~/.gyp/include.gypi
  fi
