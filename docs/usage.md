...
- [Accounts authentication](https://github.com/Dekstra/dekstra/blob/master/docs/accounts-authentication.md)
- [How do I run Dekstra in portable mode?](#portable)
- [How do I fix the default file manager?](#file-manager)
- [How do I press and hold a key and have it repeat in Dekstra?](#press-and-hold)
- [How do I open Dekstra from the terminal?](#terminal-support)
  - [From Linux .tar.gz](#from-linux-targz)

## <a id="signin-github"></a>Sign in with GitHub

In Dekstra, `Sign in with GitHub` is using a Personal Access Token.<br />
...
## <a id="portable"></a>How do I run Dekstra in portable mode?
...
- **macOS** : portable mode is enabled by the existence of a specially named folder. For Visual Studio Code that folder name is `code-portable-data`. For Dekstra, that folder name is `codium-portable-data`. So to enable portable mode for Dekstra on Mac OS, follow the instructions outlined in the [link above](https://code.visualstudio.com/docs/editor/portable), but create a folder named `codium-portable-data` instead of `code-portable-data`.

## <a id="file-manager"></a>How do I fix the default file manager (Linux)?

In some cases, Dekstra becomes the file manager used to open directories (instead of apps like Dolphin or Nautilus).<br />
...
## <a id="press-and-hold"></a>How do I press and hold a key and have it repeat in Dekstra (Mac)?

This is a common question for Visual Studio Code and the procedure is slightly different in Dekstra because the `defaults` path is different.

```bash
$ defaults write com.dekstra ApplePressAndHoldEnabled -bool false
```

## <a id="terminal-support"></a>How do I open Dekstra from the terminal?
...
This allows you to open files or directories in Dekstra directly from your terminal:
...
When the archive `Dekstra-linux-<arch>-<version>.tar.gz` is extracted, the main entry point for Dekstra is `./bin/codium`.
